...
MAP_LIST="${RC_MAP_LIST:-c0a0e c1a0}"
BOOT_TIMEOUT="${RC_BOOT_TIMEOUT:-180}"
MAP_TIMEOUT="${RC_MAP_TIMEOUT:-180}"
...
timing_compliance_gate() {
	local log_path="$LOG_DIR/timing-compliance.log"
	echo
	echo "== timing compliance =="
	if scripts/gamecube-timing-compliance.py --log-dir "$LOG_DIR/timing-compliance" >"$log_path" 2>&1; then
		log_status "timing compliance" "PASS" "$log_path" "G49 frame timing/loading/timing-independence preflight passed"
		cat "$log_path"
		return 0
	fi
	local rc=$?
	log_status "timing compliance" "FAIL" "$log_path" "exit $rc"
	cat "$log_path" >&2
	return "$rc"
}
...
release_compliance_gate() {
	local log_path="$LOG_DIR/release-compliance.log"
	echo
	echo "== release compliance =="
	if scripts/gamecube-release-compliance.py --log-dir "$LOG_DIR/release-compliance" >"$log_path" 2>&1; then
		log_status "release compliance" "PASS" "$log_path" "G52 release manifest/legal audit preflight passed"
		cat "$log_path"
		return 0
	fi
	local rc=$?
	log_status "release compliance" "FAIL" "$log_path" "exit $rc"
	cat "$log_path" >&2
	return "$rc"
}
...
reproducibility_gate() {
	local log_path="$LOG_DIR/reproducibility.log"
	echo
	echo "== release artifact reproducibility =="
	if scripts/gamecube-reproducibility-check.py --log-dir "$LOG_DIR/reproducibility" >"$log_path" 2>&1; then
		log_status "release artifact reproducibility" "PASS" "$log_path" "G55 artifact manifest and reproducibility preflight passed"
		cat "$log_path"
		return 0
	fi
	local rc=$?
	log_status "release artifact reproducibility" "FAIL" "$log_path" "exit $rc"
	cat "$log_path" >&2
	return "$rc"
}

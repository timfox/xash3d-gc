...
: "${AI_DIRTY_COMMIT_EXCLUDE:=scripts/xash3d-gc-aider-gui.py:.ai/state/xash3d-gc-aider-gui-settings.json:.ai/state/autoport-heartbeat.json:.ai/state/autoport-runner.pid:.ai/state/autoport-watchdog.pid:.ai/state/experiment-latest.json:.ai/state/discovery-supervisor.json:.ai/state/discovery-hypotheses.json}"
...
: "${AI_AUTO_DISCOVERY:=after-goals}"

gamecube_apply_local_llm_profile() {
	case "${AI_LOCAL_PROFILE:-default}" in
		rtx-pro-4000-24gb|rtx4000-24gb|24gb)
			: "${QWABLE_5_MAX_NUM_SEQS:=1}"
			: "${QWABLE_5_GPU_MEMORY_UTILIZATION:=0.72}"
			: "${QWABLE_5_MAX_MODEL_LEN:=32768}"
			: "${AIDER_SYSTEM_OVERHEAD_TOKENS:=16384}"
			: "${AIDER_MAX_CHAT_HISTORY_TOKENS:=256}"
			: "${AIDER_LOW_VRAM_PROFILE:=1}"
			: "${LOCAL_CODER_MAX_NUM_SEQS:=$QWABLE_5_MAX_NUM_SEQS}"
			: "${LOCAL_CODER_GPU_MEMORY_UTILIZATION:=$QWABLE_5_GPU_MEMORY_UTILIZATION}"
			: "${LOCAL_CODER_MAX_MODEL_LEN:=$QWABLE_5_MAX_MODEL_LEN}"
			export AIDER_LOW_VRAM_PROFILE LOCAL_CODER_MAX_NUM_SEQS \
				LOCAL_CODER_GPU_MEMORY_UTILIZATION LOCAL_CODER_MAX_MODEL_LEN
			;;
	esac
}

gamecube_apply_local_llm_profile
export OPENAI_API_KEY OPENAI_API_BASE AIDER_SERVED_MODEL QWABLE_5_MAX_NUM_SEQS \
	...
	AI_LOCAL_PROFILE AI_AUTO_DISCOVERY
